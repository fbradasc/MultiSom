...
#endif
#ifdef DISARM_AT_STARTUP // perform a disarm action at the end of initialization to create a log record
    go_disarm();
#endif
#ifdef DEBUGMSG
...
void go_inflight_pid_tuning()
{
    if (f.PIDTUNE_MODE)
    {
        ...
        if (f.ARMED)
        {
            ...
            if (pidsel > MINCHECK)
            {
                aux_pos |= 0x20;    // check for MIN
            }

            if (pidsel < MAXCHECK)
            {
                aux_pos |= 0x40;    // check for MAX
            }
...
# if GPS
            && (!rcOptions[BOXGPSNAV] && FAILSAFE_MISSION)
# endif
            )
        {
...
# endif
        ...
        if (rcDelayCommand == 20)
        {
            ...
            else                            // actions during not armed
            {
                ...
                /* START - ARMING */
#ifdef ALLOW_ARM_DISARM_VIA_TX_YAW
                if ( (conf.activate[BOXARM] == 0) && (rcSticks == THR_LO + YAW_HI + PIT_CE + ROL_CE) )
                {
                    go_arm();    // Arm via YAW
                }
#endif
#ifdef ALLOW_ARM_DISARM_VIA_TX_ROLL
                if ( (conf.activate[BOXARM] == 0) && (rcSticks == THR_LO + YAW_CE + PIT_CE + ROL_HI) )
                {
                    go_arm();    // Arm via ROLL
                }
#endif
                ...
                if (rcSticks == THR_LO + YAW_LO + PIT_HI + ROL_HI)           // Inflight ACC calibration START/STOP
                {
...
# endif
                ...
                if (rcSticks == THR_LO + YAW_LO + PIT_HI + ROL_CE)
                {
                    i = 2;    // PITCH up   -> Profile 2
                }
                else
                if (rcSticks == THR_LO + YAW_LO + PIT_CE + ROL_HI)
                {
                ...
                if (rcSticks == THR_LO + YAW_CE + PIT_HI + ROL_LO)                     // Auto telemetry ON/OFF
                {
                    if (telemetry_auto)
                    {
                        telemetry_auto = 0;
                ...
                if (rcSticks == THR_LO + YAW_CE + PIT_HI + ROL_HI)                     // Telemetry next step
                {
                ...
                if (rcSticks == THR_HI + YAW_CE + PIT_LO + ROL_CE)
                {
                    conf.angleTrim[PITCH] -= 2;
                    i = 1;
                }
                else
                if (rcSticks == THR_HI + YAW_CE + PIT_CE + ROL_HI)
                {
                    conf.angleTrim[ROLL] += 2;
                    i = 1;
                }
                else
                if (rcSticks == THR_HI + YAW_CE + PIT_CE + ROL_LO)
                {
                    conf.angleTrim[ROLL] -= 2;
                    i = 1;
                }
                ...
                if (rcSticks == THR_CE + YAW_LO + PIT_LO + ROL_LO)      // decrease P
                {
                    conf.pid[ROLL].P8 -= PID_INC;
                    blinkLED(2, 40, 1);
                    i = 1;
                ...
                if (rcSticks == THR_CE + YAW_LO + PIT_HI + ROL_CE)      // increase I
                {
                ...
                if (rcSticks == THR_CE + YAW_LO + PIT_LO + ROL_CE)      // decrease I
                {
                    conf.pid[ROLL].I8 -= PID_INC;
                    blinkLED(2, 40, 1);
                    i = 1;
                ...
                if (rcSticks == THR_CE + YAW_LO + PIT_HI + ROL_HI)      // increase D
                {
                ...
                if (rcSticks == THR_CE + YAW_LO + PIT_LO + ROL_HI)      // decrease D
                {
                    conf.pid[ROLL].D8 -= PID_INC;
                    blinkLED(2, 40, 1);
                    i = 1;
                ...
                if (rcSticks == THR_CE + YAW_HI + PIT_LO + ROL_LO)      // decrease P
                {
                    conf.pid[PITCH].P8 -= PID_INC;
                    blinkLED(2, 40, 1);
                    i = 1;
                ...
                if (rcSticks == THR_CE + YAW_HI + PIT_HI + ROL_CE)      // increase I
                {
                ...
                if (rcSticks == THR_CE + YAW_HI + PIT_LO + ROL_CE)      // decrease I
                {
                    conf.pid[PITCH].I8 -= PID_INC;
                    blinkLED(2, 40, 1);
                    i = 1;
                ...
                if (rcSticks == THR_CE + YAW_HI + PIT_HI + ROL_HI)      // increase D
                {
                ...
                if (rcSticks == THR_CE + YAW_HI + PIT_LO + ROL_HI)      // decrease D
                {
                    conf.pid[PITCH].D8 -= PID_INC;
                    blinkLED(2, 40, 1);
                    i = 1;
...
# ifndef SLIM_WING
                        ...
                        if (rcOptions[BOXLAND])                                      //Land now (It has priority over Navigation)
                        {
                            f.GPS_mode = GPS_MODE_HOLD;
...
# endif
                        ...
                        if (rcOptions[BOXGPSNAV])                                    //Start navigation
                        {
    ...
    else     // not in rc loop
    {
        ...
        switch (taskOrder)
        {
            ...
            case 1:
            {
                ...
                if (Baro_update() != 0)
                {
                    break;    // for MS baro: I2C set and get: 220 us  -  pressure and temperature computation 160 us
                }
#endif
            }
            ...
            case 3:
            {
                taskOrder++;
#if GPS
                if (millis() - GPS_FAIL_timer > 2000)
                {
                    f.GPS_FIX = 0;    // Check for lost GPS.
                }
...
# endif
    ...
    for (axis = 0; axis < 2; axis++)
    {
        ...
        if (f.ANGLE_MODE || f.HORIZON_MODE)   // axis relying on ACC
        {
            ...
            errorAngle = constrain(rc
# if GPS
                                   + GPS_angle[axis]
# endif
                                   , -500, +500) - att.angle[axis] + conf.angleTrim[axis];         //16 bits is ok here
