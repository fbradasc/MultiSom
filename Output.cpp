...
#   ifndef EXT_MOTOR_RANGE
    TC4H = motor[0] >> 8;
    OCR4D = (motor[0] & 0xFF); //  pin 6
#   else
    TC4H = ( ( (motor[0] - 1000) << 1 ) + 16 ) >> 8;
    OCR4D = ( ( ( (motor[0] - 1000) << 1 ) + 16 ) & 0xFF ); //  pin 6
#   endif
#  else
...
#   ifndef EXT_MOTOR_RANGE
    ...
    TC4H = motor[5] >> 8;
    OCR4A = (motor[5] & 0xFF); //  pin 13
#   else
    OCR1C = ( (motor[4] << 4) - 16000 ) + 128;
    TC4H = ( ( (motor[5] - 1000) << 1 ) + 16 ) >> 8;
    OCR4A = ( ( ( (motor[5] - 1000) << 1 ) + 16 ) & 0xFF ); //  pin 13
#   endif
#  endif
# endif
# if (NUMBER_MOTOR > 6)
...
#  if  defined(GPS) && defined(AUTOFLAPS)
    if (f.GPS_FIX)  // we have valid speed info
    {
        ...
        if ( ( flap < 0) && (f.ANGLE_MODE || f.HORIZON_MODE) ) // if TX set to upper half extension, activates AutoFlap on assisted modes
        {
...
#  if  defined(GPS) && defined(AUTOFLAPS)
    if (f.GPS_FIX)  // we have valid speed info
    {
        ...
        if ( ( lFlap < 0) && (f.ANGLE_MODE || f.HORIZON_MODE) ) // if TX set to upper half extension, activates AutoFlap on assisted modes
        {
